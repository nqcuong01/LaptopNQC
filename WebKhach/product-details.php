...
<head>
    ...
    <link rel="stylesheet" href="./css/3c-comments.css">
    <style>
        #cadd input[type=button] {
        background: #db4104;
        border: 0;
        color: #fff;
        padding: 10px;
...
<div class="container">
<div class="card">
    <h3 style="padding:10px;padding-bottom:0px">Bình luận</h3>
    <div id="cwrap">
    ...
    <input type="text" id="cname" name="ten" placeholder="Tên" required/>
    <input type="hidden" id="time" name="thoigian" value="<?php echo date("Y-m-d") ?>"/>
    <textarea id="cmsg" name="noidung" placeholder="Bình Luận" required></textarea>
    <input type="button" value="Gửi" id="add_bl"/>
    </form>
</div>
</div>
<!-- End code cmt -->
...
$(document).ready(function(){
    ...
    $("#add_bl").click(function(){
        ...
        var noidung = $("#cmsg").val();

        if(ten=="" || noidung=="")
        {
            alert("không đc để trống tên và nội dung bình luận");
        }
        else{
            $.ajax({    
                url:"xulybinhluan.php",
                method:"POST",
                data:{act:"add",masp:masp,ten:ten,thoigian:thoigian,noidung:noidung},
                success:function(data)
                {
                    load_binhluan();
                }
